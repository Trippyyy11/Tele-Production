...
import { getTasks, exportAnalytics, updateTaskMetrics, retryTask } from '../services/api'
import {
    Eye, Forward, MessageSquare, Heart,
    TrendingUp, RefreshCw, Download, Calendar,
    User, Hash, CheckCircle, Shield, Search, Filter
} from 'lucide-react'
...
function DataTracking() {
    ...
    const flattenedData = tasks.flatMap(task =>
        (task.sentMessages || []).map(msg => ({
            ...
            status: task.status === 'undone' ? 'undone' : task.status === 'expired' ? 'expired' : task.status === 'failed' ? 'failed' : 'sent',
            ...
            sentBy: task.createdByUsername
        }))
    )
        .filter(item => {
            const matchesSearch = item.taskName.toLowerCase().includes(search.toLowerCase())
            const matchesUser = !filterUser || item.sentBy === filterUser
            ...
            if (dateRangePreset === 'today') {
                matchesDate = itemDate >= now
            } else if (dateRangePreset === 'yesterday') {
                const yesterday = new Date(now)
                yesterday.setDate(now.getDate() - 1)
                matchesDate = itemDate >= yesterday && itemDate < now
                ...
                const matchesDateTo = !filterDateTo || itemDate <= new Date(filterDateTo + 'T23:59:59')
    ...
    const handleClearHistory = async () => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/tasks/history`, { method: 'DELETE' })
            if (response.ok) {
                refetch()
            } else {
                ...
                <div className="flex flex-wrap gap-3 w-full md:w-auto">
                    <div className="flex flex-col lg:flex-row gap-2 w-full lg:w-auto">
                        <input
                            ...
                            className="flex-1 px-3 py-2 rounded-xl bg-white border border-gray-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-100 focus:outline-none text-sm"
                        />
                        <select
                            value={filterUser}
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="text-xs uppercase tracking-widest text-gray-500 font-bold bg-gray-50">
                                ...
                                <th className="px-4 py-4 border-b border-gray-100 min-w-[120px]">Sent By</th>
                                ...
                                flattenedData.map((row, idx) => (
                                    <tr key={`${row.taskId}-${row.chatId}-${row.messageId}`} className="hover:bg-gray-50 transition-colors group cursor-default">
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="flex items-center gap-2">
                                                <User className="w-3 h-3 text-gray-400" />
                                                <span className="text-xs font-medium text-gray-700">{row.sentBy || '-'}</span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-4">
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            <div className="flex items-center justify-center gap-2">
                                                {row.status === 'failed' && (
                                                    <button
                                                        onClick={() => handleRetry(row.taskId)}
