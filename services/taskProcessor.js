...
const processTask = async (taskId) => {
    ...
    try {
        ...
        if (task.targetIds && task.targetIds.length > 0) {
            // Use explicit ordered targets if available
            uniqueRecipients = task.targetIds;
            console.log(`🎯 Using explicit target list (${uniqueRecipients.length} recipients)`);
        } else {
            // Fallback to folder-based collection
            const recipientIds = [];
            ...
            // De-duplicate
            uniqueRecipients = [...new Set(recipientIds)];
        }

        console.log(`👥 Sending to ${uniqueRecipients.length} recipients`);

        // 5. Trigger Telegram Broadcast
        // Extract scheduling from content if present (where we stored it in the route)
        const schedulingConfig = task.content.scheduling || {};
        ...
        if (results.failed > 0) {
            ...
            results.errors.forEach(e => {
                log(`   - ${e}`);
                try { require('fs').appendFileSync(path.join(__dirname, '..', 'debug_error.log'), `[TASK FAILURE] ${e}\n`); } catch (e) { }
            });
        }
        ...
        try { require('fs').appendFileSync(path.join(__dirname, '..', 'debug_error.log'), `[TASK CRASH] ${err.message}\n${err.stack}\n`); } catch (e) { }
